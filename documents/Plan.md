...
* Заполнение поля «Номер карты»
    ...
    * Ввод карты не из набора с валидным номером.
...
* `IntelliJ IDEA` (Ultimate/Community): Основная среда разработки.
* `Java 11` Язык написания тестов.
* `Gradle` Инструмент сборки и управления зависимостями.
* `Selenide` Фреймворк для UI-тестов. 
* `Faker` Библиотека для генерации случайных данных (имена владельцев, тестовые данные).
* `Lombok` Библиотека сокращает объем стандартного кода в классах.
* `Docker` Для контейнеризации и быстрого развертывания СУБД и Node.js эмулятора.
* `Allure` Для генерации наглядных отчетов о прохождении тестов.

## 3. Перечень и описание возможных рисков при автоматизации.

* Проблемы с портами: если порты заняты другими программами, ничего не запустится.
* Различия в СУБД: необходимость поддержки двух диалектов SQL (MySQL и PostgreSQL) может усложнить код тестов для проверки БД.
* Тест может упасть из-за случайной задержки анимации или медленного ответа симулятора.
* Трудности поиска элементов: если у полей нет четких ID, тесты будут часто терять нужные кнопки.

## 4. Интервальная оценка с учётом рисков в часах

* Изучение технического задания, подготовка окружения проекта, подключение библиотек, зависимостей, запуск и изучение приложения – 12 часов.

* Разработка и написание плана автоматизации тестирования - 8 часов.

* Написания кода и отладка автотестов - 48 часов.
...
* Проведены автотесты `16.04.2026`
* Отчётные документы по итогам тестирования `17.04.2026 - 18.04.2026`
* Отчётные документы по итогам автоматизации `19.04.2026`